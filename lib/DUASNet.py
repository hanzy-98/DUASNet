...
class All_in_one_SegNet(nn.Module):
    
    def __init__(self, channels=256, output_stride=16, pretrained=True):

        super(All_in_one_SegNet, self).__init__()

       
        self.resnet = res2net50_v1b_26w_4s(pretrained=pretrained, output_stride=output_stride)

        
        self.context2 = MES(512, channels)
        self.context3 = MES(1024, channels)
        self.context4 = MES(2048, channels)
 
        self.decoder = ESAB_decoder(channels)


        self.attention2 = FFB(channels * 2, channels)
        self.attention3 = FFB(channels * 2, channels)
        self.attention4 = FFB(channels * 2, channels)
